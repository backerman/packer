...
- `skip_root_mount` (bool) - Skip implicitly mounting and unmounting the root volume. This option can be used for filesystems that
...
- `cleanup_commands` ([]string) - As `pre_mount_commands`, but the commands are executed immediately before the disk is detached.
